...
nTR    = numel(TR_exp); 
% 4) Choose parameterization of phase signs of single compartment profiles
sigmaTR    = 1;
sigmaTE    = -1; 
sigmaphi   = 1; 
GlobalPhase = 1000*randn(1);    %randomized global phase factor does not change shapes at all!
...
for indTR = 1:nTR
    ...
    for indPC = 1:nPC
        profile_water(indPC) = S_bSSFP_SignParameterization(M0,T1,T2,alpha,phi(indPC),TR,TE,deltaCSw,dB0,B0,sigmaTR,sigmaTE,sigmaphi,GlobalPhase);
        profile_Ace(indPC)   = S_bSSFP_SignParameterization(M0,T1,T2,alpha,phi(indPC),TR,TE,deltaCS+deltaCSw,dB0,B0,sigmaTR,sigmaTE,sigmaphi,GlobalPhase);
    end
...
% saved figures for following configurations
% sigmaTR    = 1;
% sigmaTE    = -1; 
% sigmaphi   = 1; 
% GlobalPhase = 1000*randn(1); 
% Note: sigmaphi   = -1; does not change the shapes...
if 1 ==2
    if indTR == 1
        saveas(gcf,'Note_Simulation_3p4_opposite_sigmaTRpos.png')
    elseif indTR == 2
        saveas(gcf,'Note_Simulation_3p62_opposite_sigmaTRpos.png')
    elseif indTR == 3
        saveas(gcf,'Note_Simulation_4p11_opposite_sigmaTRpos.png')
    elseif indTR == 4
        saveas(gcf,'Note_Simulation_4p62_opposite_sigmaTRpos.png')
    elseif indTR == 5
        saveas(gcf,'Note_Simulation_4p84_opposite_sigmaTRpos.png')
    elseif indTR == 6
        saveas(gcf,'Note_Simulation_5p14_opposite_sigmaTRpos.png')
    end
end
